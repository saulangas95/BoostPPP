...
When running `BoostPPP_IEEE.mlx`, the following files may be generated (depending on the export lines enabled in the script):

- `Fig4_PoleZeroMigration.pdf` (vector PDF)
- `Fig8_kFactor_and_PSQ.pdf` (vector PDF)
- `Fig10_Gvd_Bode.pdf` (vector PDF, optional)

> **Note:** The script uses `Renderer = painters` and `exportgraphics(...,'ContentType','vector')` for publication-quality figures.
